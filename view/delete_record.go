...
func DeleteRecord(ctx *gin.Context) {
	...
	var reqPay handler.DeleteRecordRequest
	var resPay handler.DeleteRecordResponse
	...
	if err := handler.DeleteRecord(req); err != nil {
		slog.Error("error while deleting record", "error", err.Error())
